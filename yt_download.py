...
def video(url, output='mp4', quality='best'):
    ydl_opts = {
        'format': f'{output}+bestvideo[height<=?1080]+bestaudio/best[height<=?1080]',
...
def audio(url, output='mp3', quality='320k'):
    ydl_opts = {
        'quiet': False,
        'verbose': True,
...
if __name__ == '__main__':
    ...
    output_format = input("Enter output format (mp4, avi, mkv, mp3, etc.): ")

    quality_type = input("Enter quality (e.g. 720p, 1080p, 192k, etc.): ")

    match choice:
        case '1':
            video(link, output=output_format, quality=quality_type)
        case '2':
            audio(link, output=output_format, quality=quality_type)
        case _:
